          ...
          attempts:
            - id: gate-attempt:drc-4044-triage-1
              ...
              application:
                ...
                state: consumed
        - id: gate:drc-4044:review
          stage: review
          attempts:
            - id: gate-attempt:drc-4044-review-1
              briefing:
...
Twelve criteria. AC1-AC9 belong to PR 1, AC10 and AC12 to PR 2, AC11 to the issue. AC11 is the only
...
no implementer, since PR 1 publishes `history_reset` at `aggregate.py:665` and nothing reads it.
...
**Cycle 3 (review of PR 2, 2026-09-04): two lenses plus an arbiter.** From AGENTS.md's Calibrating
Effort table, row "Owns a conflict-prone surface (`web/` byte pins, `SKILL.md`, `config.py`)". **The
diff property that justified it:** PR 2 is the only PR of this issue that touches
`cargento_runtime/web/`, and it owns *both* frontend byte-pin oracles — `test_next_page.py`'s
per-part figures and `test_next_flag.py`'s assembled pair. It is not the security/data-loss row that
set PR 1's tier: the persistent store, its never-list and its bounds all shipped in PR 1 and are not
re-opened here. The +36 runtime lines outside `web/` are a label cap moving to its source, which
narrows what is retained rather than widening it. **Shape declared before the first spawn:** 2
lenses (the seeded panels' truthfulness; the label bound and M-1 end to end) and 1 arbiter
reproducing every finding at `eb45b9d` in throwaway copies — 3 agents against PR 1's 7, on a change
one third its size that re-opens no contract.

The arbiter earned the tier again: it **downgraded or refuted 4 of the 6** findings the lenses
raised. Lens A's HIGH became Deferred risk once reachability was measured rather than asserted — the
lens reproduced it at 12,000 records, above the shipped 1 MiB ceiling of ~8,256; the real bar is a
saturated store *plus* ~13 sustained-concurrent sessions. Lens B's Material-shaped claim became
Deferred risk because all three call sites are correct at head, so the harm is future regression
rather than present loss. One finding was reproduced and then refuted as a defect outright.
...
## Stage Report: implementation (PR 2)

- DONE: PR 2 built test-first in a fresh worktree from `main` (which now carries PR 1 at `ff8280a` and DRC-4344 at `0f0d4a7`): `next-workstream.js` seeded from the published history field with a truthful caption, `next-delegation.js`'s six-hour window clamp widened to the store's window, `next-render.js` wiring, and the header reporting a store reset with its distinguishable reason (AC12, read from `history_reset`), each render test watched to fail first, the byte pins recomputed from the assets in both `test_next_page.py` and `test_next_flag.py`, and no file outside `cargento_runtime/web/` and its tests except the carried items below
  Six commits, head `d66d396`. **AC10:** the rail replays `history` on the first payload, each stored record becoming a batch of every session known at that stamp; watched red first (`[] != two transitions`). **AC12:** `next-chrome.js` renders a reset notice keyed on the two literals; watched red first, and the falsifier the AC names — a header identical for both — is what the red *was*. **Three caption sites, not one, and running the mutation is what found that:** AC10's own falsifier (seed the data, hardcode the caption) reddened only the rail header; the empty-rail paragraph and the delegation panel's withheld header kept their literals green. Two oracles added at `d66d396`, and all three sites now red under their own hardcode mutation. `next-render.js` is **SKIPPED, and deliberately**: it already calls `nextObserveWorkstream(fresh)`, so seeding inside that function needed no wiring and editing the file would have been a no-op commit. **Two defects found reviewing the diff in the worktree**, both fixed with a watched red: a seeded observation held its state until this tab's first payload, so a session left `working` when the laptop closed manufactured **17,200 s of delegated time** (`562acc3` — a session's last stored record now closes its span rather than opening one, biasing down per NUI-11 instead of inventing availability); and the store keeps no token rate, so dividing the measured area by every delegated second printed **≥4 tok/m where the measured rate was 100** (the denominator is now the span that was measured). Byte pins recomputed from the assets three times as the code moved, never from a number in a report: `next-workstream.js` 17,492, `next-delegation.js` 8,583, `next-chrome.js` 17,327, assembled page 332,948, in both oracle files.
- DONE: The items carried from PR 1's cycle-2 review landed or declined with a reason each: DR-8 `_bounded_project` trimming a correct hyphenated label one directory under `$HOME` (one line plus the docstring sentence), DR-9 AC4's `open()`-then-`chmod` falsifier made to red by asserting the mode argument in the spy, DR-10 AC2's sentinel made dash-free so the two-segment trim cannot defeat it, the dead `OverflowError` copy in `_decode` removed or commented as unreachable, the AC preamble count and the `history_reset` line citation corrected on the entity, PR #260's stale verification figures noted as PR 1's; DR-11 (the refusal probes the port not the store) and DR-12 (a cap below one record strands an over-cap store) recorded with their promote conditions, not changed
  **DR-9 landed:** the temp-file spy records the mode argument; the AC's named mutation (`os.open` without the mode, then `os.chmod`) now reds `384 != None` in a throwaway copy, where it was green. **DR-10 landed:** sentinels use `_`, neither separator the bound splits on; routing the Spacedock sentinel into `project` through `_bounded_project` now reds, where the trim used to hide it as `SPACEDOCK-e8`. **`OverflowError` commented as unreachable** rather than deleted, naming why (`float()` returns `inf`, `math.isfinite` raises ValueError) and where the reachable copy is (`Lane._open`); one class in a tuple whose absence took a live server down once is not worth the deletion. **Entity corrected:** the preamble now reads twelve criteria with AC1-AC9 to PR 1 and AC10/AC12 to PR 2, and the citation is `aggregate.py:665` — verified by grep, and **the review's own correction to 659 was itself wrong**, 659 being `fields["history"]`. **PR #260 noted, not written to:** its `## Verification` block already carries a "the head has moved twice since these figures" bullet with the corrected 1,857 / 90.4%, so the item is satisfied in the artifact; its remaining `aggregate.py:655` citation is 665 and PR 2's body says so rather than editing a merged PR. **DR-11 and DR-12 recorded, unchanged**, with the promote conditions the review gave them. **DR-8 is FAILED as scoped and open with the FO** — see the next line.
- FAILED: DR-8 fixed so the panels group history under the same label as the live board
  It cannot be done inside `_bounded_project`. That function picks its separator textually, and `my-cool-project`, `recce-cloud-infra` and `spacedock-ensign-drc-4044` are the same shape to it as M-1's dash-encoded path — reproduced on the branch, they store as `cool-project`, `cloud-infra` and `drc-4044`. No segment-count threshold separates them either: `docs-sync-next-ui-design-parity` is a real worktree directory here with six dash segments. Dropping the dash branch is the one line the checklist sizes and it re-opens M-1, a Material finding against `contract[SECURITY.md#local-history-the-session-history-store]`, which is not mine to revert. Put to the FO with two options — bound at the three collectors' `project_label` fallback where provenance is known (~4 runtime files, and it changes what the board displays for the 0.75% of rows that hit the fallback), or decline and file it. **Unanswered at the time of writing.** The user-visible consequence if declined: seeded history does not render for a project directly under `$HOME`, because the store's label and the board's disagree there.
- FAILED: the surface measured against the PR 2 estimate (web +60/3 files ±25%, oracles +150/3 ±25%) before the PR opens — stopping and putting it to the FO if beyond tolerance — the diff reviewed in the worktree, and a PR opened whose body starts `Implements [DRC-4044](https://linear.app/recce/issue/DRC-4044) — H1 · Keep a history of what happened` with `## Verification`, its number and head SHA reported
  Measured before the PR opened, beyond tolerance on both rows, so I stopped and put it to the FO rather than opening: **`web/` +170 net across 3 files** against 45-75, and **oracles +346 net across 6 files** against 112-188, plus +4 runtime and +23 docs that the PR 2 estimate has no row for. Attributed: AC10 is the bulk, AC12 (the ND-4 ruling, added after the estimate) is web +27 / oracles +48, the carried review items are +4 / +14. **The estimate modelled the delegation panel as clamp-gated; it is batch-gated** — `nextDelegationRange` integrates sample batches, the store keeps transitions, and **exercised rather than reasoned**: with the widened clamp in place and the seeding pass made to build no batches, the seeded test still renders `no figure yet`. Reconstructing a batch per stored record is what AC11's second half actually costs. **The diff was reviewed in the worktree** and that review is what found both defects above and the two missing caption oracles, which is the whole argument for the step. **No PR opened**, pending the ruling.
- DONE: The canonical pre-PR suite from `AGENTS.md` run once with any red in the four contention-prone modules re-run alone, `sync-docs` invoked with its commit on this branch
  Run once at load 6.3 with six sibling worktrees live: `ruff check` passed, `ruff format --check` 151 files, `mypy` clean over 112 files, `lint_embedded` clean, `validate_plugins` OK, `bump_version --current` 0.20.0, merge-base diff over the version fields **empty**, **1,894 tests OK (1 skipped) in 36 s**, 209 script tests OK, `coverage report` **90.2%** against `fail_under` 73. **No red in any module, so nothing needed re-running alone** — `test_http_api`, `test_page`, `test_lifecycle` and `test_quota` all passed on the single run. `sync-docs` invoked and committed at `d02ee25` and `49208b2`: NUI-9 and NUI-11 rewritten (both said the ledger had no source but this tab, in their headings), the module map given the seeding and the reset notice, `SKILL.md` told that the two panels survive a restart, and three em dashes left by PR 1 in `HOW_TO_USE.md` and `SECURITY.md` brought back to the prose standard. The marker was left alone per the parallel-work rule. Linear: the body's "Eleven" patched to "Twelve"; relations read back **unchanged** (blocks 5, blockedBy 0, relatedTo 5); one serializer boundary move in pre-existing text (`**Waits on** ` became `**Waits on **`), reported and **not repaired**, because repair provably cannot succeed.

### Deferred risk raised this round

- **DR-13 — a server stopped mid-store still contributes its gap.** `562acc3` closes the tail, where a session's last stored record has no observed end. A server stopped *between* two recorded transitions of the same session is different: the store carries no evidence separating that from a session whose state simply did not change, and any hold cap would be a guess. *Promote if* a stop or start marker ever reaches the store, at which point the gap becomes observable and the seeded span can be cut at it.

### Equivalent mutant, named rather than buried

Mutating `nextWorkstreamObservedSince` back to `generated` leaves every test green, and it is not a gap. `nextWorkstreamProjectWindow` derives a project's start from the groups carrying that project, so the module-level fallback is only reachable for a project with no groups at all. The three caption oracles above are the ones that bind.

### Summary

AC10 and AC12 are implemented and pinned, each render test watched red first and each caption site red
under its own hardcode mutation. Two of the three carried oracle items landed with their named
falsifiers reproduced in throwaway copies; the `OverflowError` copy is commented as unreachable; the
entity's two citations are corrected, one of them against the review's own wrong correction.

Two things are open and both are with the FO. **DR-8 cannot be fixed as the checklist sizes it** —
`_bounded_project` has no textual discriminator between a hyphenated directory name and a
dash-encoded path, and the one-line fix re-opens a Material finding. **The surface is beyond
tolerance on both rows**, because the estimate modelled the delegation panel as clamp-gated when it
is batch-gated; that is exercised, not asserted. Per the stage definition I stopped rather than
opening the PR.

**AC11 is owed to review** and is not claimed here: it is the interactive drive — restart the server,
open a fresh tab, confirm the rail lists the earlier transitions and captions itself with the real
...
## Stage Report: review (PR 2)

- DONE: Review depth stated up front as two lenses plus an arbiter (AGENTS.md conflict-prone-surface row: `web/` byte pins), fan-out declared before the first spawn, each lens one question — (a) the seeded panels' truthfulness; (b) the label bound at its three source call sites and the `_bounded_project` simplification with M-1 re-verified end to end — and the arbiter reproducing every finding against head `eb45b9d` in a throwaway copy
  Depth and its justifying diff property written into `## Review depth` above as Cycle 3; 3 agents declared before the first spawn and 3 spawned. Every agent worked in `git archive` copies of `eb45b9d`; the worktree is clean (`git status --porcelain` empty at report time). The arbiter **downgraded or refuted 4 of 6**: lens A's HIGH fell to Deferred risk when the 1 MiB ceiling was measured (~8,256 records) against the lens's 12,000-record reproduction, and lens B's claim fell to Deferred risk because all three call sites are correct at head.
- DONE: AC10 and AC12 reproduced from their own `Verified by:` clauses on head `eb45b9d` — the six byte pins recomputed by me from the assets in both pinning files, the caption mutation applied at all three sites, the seeded-window and denominator falsifiers re-applied
  **Six pins recomputed from the assets, not from any report** (`git cat-file` for the parts, `frontend_page.load_page()` for the page): `next-chrome.js` 17,327/`5101046a…`, `next-workstream.js` 17,492/`a19c7213…`, `next-delegation.js` 8,583/`e526ce96…`, assembled 332,948/`49b12a44…` — all four match, and both `test_next_page.py` and `test_next_flag.py` carry the same assembled pair. **Caption mutation applied by me at all three sites**, each red on its own: rail header (3 failures), empty-rail paragraph (1), delegation header (4). **Four named falsifiers re-applied, all red:** the 17,200 s defect (last record opens rather than closes a span) 2 failures; denominator back to `delegatedSec` 1; six-hour clamp restored on a seeded window 1; AC12's identical-header-for-both-reasons 1. Baseline 63 tests green before and after each.
- DONE: AC1-AC9's oracles confirmed still green, and AC4's state freeze confirmed by a two-collector live comparison since the label bound touches three collectors
  **Suite run once** at load 4.07 in a fresh copy of `eb45b9d`: **1,895 tests OK (1 skipped) in 34.9 s**. No red, so nothing needed re-running alone. Script tests 209, one error — `test_root_docs_lists_every_prose_doc_in_the_repository`, which shells `git ls-files` and exits 128 in a `git archive` copy; **re-run in the real worktree it passes**, so it is my artifact, not the PR's. **AC4 live:** a server pointed at a never-created state home made the directory `0o700` and its files `0o600`, no temp file left behind. **Two-collector comparison, live against the real harness stores:** claude 6 + codex 1 in the store, **7 of 7 store labels equal the published row label, 0 disagreements, 0 labels deeper than two segments on either side** — M-1's retention property holding end to end after the bind moved to the source.
- DONE: **AC11 driven live by me** — the head served from a spare port against the same stores, a project observed across state changes, that server stopped and restarted, a fresh tab opened, the rail listing the earlier transitions with the real window caption and the delegation figure rendering; then the negative arm under `--no-history`
  `Verified by: live session:docs/roadmap-burndown/.spacedock-state/drc-4044/captures/ac11-live-drive-2026-09-04.jsonl` (11 records; field names, counts and booleans only, no operator text). Head served on **port 4571** with an isolated `CARGENTO_HOME`, against the **real** harness stores; the captain's server on 4553 was never touched and its store is byte-identical (25 records, original mtime). **Positive arm:** server 1 observed 7 real sessions and wrote a `0o600` store; it was stopped, the store survived process exit, a second server started and a **fresh tab** opened — the rail listed **12 earlier transitions** captioned **`last 22h 49m`** and the delegation figure **rendered 100% captioned `LAST 22H 48M`** with the `≥` rate-floor marker, on a tab seconds old. **Negative arm under `--no-history`:** the `history` field is absent from the payload, the rail is empty (`No state changes observed in the last 3s.`), the figure is withheld (`no figure yet`) and the caption returns to `SINCE THIS TAB OPENED`; the store was left **byte-identical** (same md5, same mtime). **AC12 driven live too:** a truncated store published `history_reset:"unreadable"` and rendered "The saved file could not be read."; a `v:99` store published `"version"` and rendered "It was written by a different version of Cargento." — **two different sentences**, which is what D1 exists for. **Declared honestly:** the live machine produced **zero** state changes in 40 minutes (~1 record/hour observed, and a `codex exec` one-shot never reached the store), so the 12 transitions are **scripted records carrying the real observed session identities**, appended to the 7 live-observed ones. The recording lane itself is PR 1's ground, green in `test_observation.py`; what this arm grades is the render path, which is entirely real.
- DONE: CI on head `eb45b9d`, `mergeStateStatus`, Copilot confirmed absent, the surface re-derived against the merge base
  **All 12 checks pass and every one reports `head_sha` `eb45b9d`** (verified through the check-runs API, not the summary): quality-gate, validate, version-guard, latest-client-smoke, Detect, Lint, mypy, Runtime floor 3.11, Tests+coverage, and platform-tests on ubuntu/macos/windows. `mergeStateStatus` **CLEAN**, `mergeable` MERGEABLE. **Copilot absent** — `reviews` is empty and the only comment is the coverage bot, so there are no inline comments to read. **Surface re-derived** against merge-base `0f0d4a7`: `web/` **+170/3**, oracles **+404/8**, runtime outside `web/` **+36/5**, docs **+44/7** — all four rows reproduce the implementer's table exactly. Beyond the estimate on both estimated rows, accepted under `fo-ruling[2026-09-04]`; size was not my question and I did not re-litigate it.
- DONE: the carried items from PR 1's cycle-2 review (DR-8 through DR-12, four Polish) each confirmed landed or recorded with a reason
  **DR-8 landed** — `sessions.bounded_project_label` bound at the three collector fallbacks; confirmed live by the 7-of-7 comparison above. **DR-9 landed** — the temp-file spy asserts the mode *argument*, and re-applying AC4's named mutation (`os.open` without the mode, then `os.chmod`) **reds**. **DR-10 landed** — sentinels are `SENTINEL_TITLE_e1` shaped, using `_`, neither separator the bound splits on. **DR-11 and DR-12 recorded, unchanged**, with their promote conditions. **Four Polish:** the `OverflowError` copy is commented as unreachable, naming why and where the reachable copy is; the AC preamble reads "Twelve criteria" with AC1-AC9 to PR 1 and AC10/AC12 to PR 2; the citation is `aggregate.py:665`, which I confirmed by grep is `fields["history_reset"] = notice`; PR #260 was annotated rather than edited.
- DONE: a GO or NO-GO verdict with findings under their disposition labels, no edit to the branch, no merge, no worktree or branch removed
  **GO.** Six findings, none Material — see below. I made **no edit to the PR branch** (worktree `git status --porcelain` empty), **did not merge**, and **removed no worktree or branch**. Five servers I started are stopped and port 4571 is free; no stray process remains.

### Findings under their disposition (PR 2)

**Deferred risk — DR-14: after buffer eviction the rail caption names a window wider than the rows it holds.** `next-workstream.js:344-351` with `:130-147` — `nextWorkstreamSeededSince` is populated for every seeded record and wins in `nextWorkstreamProjectWindow`, but it is not eviction-aware, while `nextWorkstreamObservedSince` is. *Released user and normal workflow:* yes, default flags. *Observable harm:* the caption overstates by ~2-3 days; no data loss, no boundary crossed. *Affected value AC:* `value-ac[AC10]`, "reports the true window rather than the tab's". *Trigger evidence:* the arbiter measured the store's real ceiling by serialising real-shaped records through `history._store_bytes` — 125 B each, **8,256 records in 1 MiB** — then drove the seeding entry point: no eviction at 12 sustained-concurrent sessions (98,930 entries), eviction at 13 (99,996), caption `last 14d` against an oldest listed row 12.12 d back. Lens A's original 12,000-record reproduction is **above the shipped ceiling**, which is why this is not Material. *Promote to Material if* `HISTORY_MAX_BYTES_DEFAULT` rises, `NEXT_WORKSTREAM_ENTRY_CAP` falls, or a real store is observed at cap with ≥13 average live sessions.

**Deferred risk — DR-15: the droid and gemini label binds have no reddening test.** `collectors/droid.py:56`, `collectors/gemini.py:113`. *Released user and normal workflow:* yes. *Observable harm:* **none today** — the arbiter confirmed all three call sites bind correctly at head and no raw `project_label(` call site remains. The harm is a future one-line regression silently re-opening a Material invariant. *Affected boundary:* `contract[SECURITY.md#local-history-the-session-history-store]` (M-1). *Trigger evidence:* full suite run three times with one revert each — `claude.py` unbound **FAILS (3)**, `droid.py` unbound **OK**, `gemini.py` unbound **OK**; driving the real droid collector with a cwd-less transcript under an 11-segment encoded directory put the whole home-relative path into the store with the suite still green. `history._bounded_project` now splits on `/` only, so the second line of defence that used to catch the dash form is gone by design. *Promote to Material if* either fallback is edited. Remediation is two ~5-line tests mirroring `test_claude_project_falls_back_when_transcript_has_no_cwd`.

**Polish — P-1: the days bucket has no oracle at a non-integral day boundary.** `next-workstream.js:369-371`; mutating `Math.floor(seconds / 86400)` to `Math.ceil` leaves all 63 panel tests green and relabels a 3d 12h window `last 4d 12h`. Not an equivalent mutant — every seeded fixture uses exactly 3d, so only `hours === 0` is pinned. One fixture at a non-integer span closes it.

**Polish — P-2: the two panels can print different windows for the same project.** Rail `last 10d` beside delegation `LAST 2D` when a project's oldest record is close-only. Each is correct under its own documented definition — the rail anchors on the store's first observation (pinned by `test_a_seeded_rail_with_no_transitions_names_the_window_it_found_none_in`), delegation refuses unclosed spans per NUI-11. Cross-panel caption agreement is pinned by no AC. Working as documented.

**Polish — P-3: one test comment asserts a grouping property the code does not have.** `tests/test_claude.py:1104-1109` says the bound "is what makes this row group with the same directory's cwd-derived rows"; grouping is exact-string (`sessions.py:459`), and `spacedock/subspace` (with cwd) and `spacedock-subspace` (without) remain two groups. The `sessions.py` docstring and the PR body both get it right — only the comment overclaims. One sentence.

**Polish — P-4 (reproduced, then refuted as a defect): `PROJECT_SEGMENT_CAP` duplicated with no parity oracle.** `sessions.py:48` and `history.py:76`, no test references either. The duplication is deliberate and documented (history is a leaf and may not import `sessions`) and the two constants govern **different separators**, so drift would not by itself be a bug. No oracle warranted.
...
- DONE: The six `## Post-merge Linear reconcile` steps run in order against the merged state (PR #260 at `ff8280a` and PR #262 at `4de75d2`, both merged; H1 is complete)
  Both merges verified on `main` at the SHA rather than from `gh pr diff`: `ff8280a` 2026-09-03T12:25:54Z, `4de75d2` 2026-09-04T02:18:52Z.
  Step 1 — verified, not set. Already `Done`, `completedAt` 2026-09-04T02:18:55.211Z, 3.2 s after PR 2 merged, so the integration moved it; the `stateHistory` shows the PR-1 premature `Done` and the correct move back to `In Progress`. No state write from this stage.
  Step 2 — **three** milestones corrected, not the two the dispatch named, each built by script from a fresh `get_milestone` capture with an exactly-once assertion and a survivor check proving every other line byte-identical and in order. `Move up a level` 2 hunks / 2 authorized edits, 17 survivors, returned body **byte-identical** (sha256 `04aef250…` sent and returned). `Don't burn capacity` 1 hunk, 16 survivors. `Spend attention well` 1 hunk, 19 survivors.
  Step 2, the third milestone and why — the dispatch placed two H1 "Waits on" lines inside `Move up a level`; the fresh capture shows **one**. Exactly two existed board-wide, the second in `Spend attention well`, which owns D10 and G4 — the two issues this same reconcile releases. Leaving it publishes a milestone asserting H1 gates two issues while they sit unblocked in `Todo`. Scripted, one hunk, and flagged in the receipt for the FO to ratify or revert.
  Step 3 — re-derived from live queries, not adjusted; three figures moved. Open issues 48→50, defects or engineering 25→27, by-move `none` 15→17, all three accounted for by DRC-4350 and DRC-4351 (filed after the previous refresh, both `move:none`, neither carrying a `release:*` label). Re-derived identical and left untouched: 48 board items, 23 open / 25 closed, seventeen shipped and eight cancelled, r1 complete, r2 two, r3 nine, `later` twelve, keep 6, sharpen 7, extend 13, new 7. Board items derived as the issues carrying a `release:*` label, which reproduces the block's own independently-written 48 exactly (23 + 17 + 8) — that is what makes it more than a restatement. The previous refresh counted H1 as shipped while it was prematurely `Done`; recomputation confirms those figures were right rather than inheriting them.
  Step 4 — all five free, all five `Backlog`→`Todo`. A6 (DRC-4009), D10 (DRC-4033), F2 (DRC-4042), F3 (DRC-4043), G4 (DRC-4051) each carried H1 as their **only** `blockedBy` edge. D10 was read for a DEC-related blocker specifically, per the dispatch, and has none. Confirmed by a post-write `unstarted` query: 8 before, 13 after, the five added being exactly those.
  Step 5 — all five `blocks` edges converted to `relatedTo`; every blocked side is open, so no closed-to-closed edge was swept, and there were none to sweep. Read back: `blocks: []`, `blockedBy: []`, `relatedTo` 7→12, exactly the five. The call returned **no error**, which is a new data point against the three prior observations of `Failed to remove N relation(s)` on a landed write.
  Step 6 — `move:new`, and no promise wording changes. Proved against the merged tree rather than assumed: no sentence in `docs/promise-map.md` or the README lede claims Cargento keeps no history or persists nothing between runs, so H1 falsifies none. PR 2 *did* edit the P2 backing clause, but with **DRC-4344's** teammate-pill wording — that was DRC-4344's named debt, not H1's. Nothing handed to `sync-docs`.
- DONE: A receipt comment posted on DRC-4044 naming all six edits, both PRs and both merge commits, the five dependents and what each became, the four follow-ups filed from H1's cycle (DRC-4345, DRC-4346-4348 where they touch H1, DRC-4350), with the relation set read back after every write and every emphasis-boundary move or unrequested edge reported rather than repaired
  Comment `c1935743-91ee-456d-be77-3edd2b8b4e61`, posted 2026-09-04T02:31:15.712Z; the returned body is byte-identical to what was sent, so the receipt took no emphasis damage.
  Follow-ups reported as **three, not four**, because that is what H1's cycle produced: DRC-4345, DRC-4350, and DRC-4351 (the DR-14 caption residual, already filed 2026-09-04T02:20:23Z, so this stage filed nothing and did not double-file). DRC-4346/4347/4348 came out of DRC-4344's cycle and do **not** touch H1 — the checklist's "where they touch H1" is what licensed saying so rather than claiming them.
  Emphasis-boundary moves: **zero**, on every write, each proven by comparing the returned body to the intended one rather than by eye. Text content, bold runs and code spans identical everywhere.
  Serializer artifact reported and **not** repaired: `Don't burn capacity` returned one bare href wrapped in angle brackets, in pre-existing text the edit did not touch; `Spend attention well` returned **all eleven** of its bare hrefs wrapped. Proven to be the only difference by a diff that normalises href delimiters — identical once normalised.
  **Unrequested edges: three, and they refute a prior narrowing.** The receipt comment added `relatedTo` to DRC-4346, DRC-4347 and DRC-4348 (`relatedTo` 12→15). DRC-4344's closeout recorded that a receipt comment naming four identifiers added zero relations and offered it as a narrowing; this contradicts it, so a comment is **not** a safe place to name an issue. Not Material: no `blocks` or `blockedBy` edge was created — `blocks: []`, `blockedBy: []` on the final read-back — which is the condition the workflow rule marks Material immediately. Reported, not repaired.
- DONE: `## Stage Report: review (post-merge closeout)` written on this entity — a new heading — with DONE/SKIPPED/FAILED per item above and every evidence line on its own line; frontmatter untouched (`reconciled` is the FO's stamp) and `merge guard` not run
  This section, appended at the end of the entity after line 1710. Frontmatter not opened; `merge guard` not invoked; no repository file written, and the throwaway worktree the dispatch helper recreated was left untouched.

### Three things for the FO

- **The dispatch's milestone count was one short, and the miss was load-bearing.** It located two H1 "Waits on" lines in `Move up a level`; one is there, and the second is in `Spend attention well` — the milestone owning D10 and G4. Correcting only the two named milestones would have left the board internally contradictory in the exact shape `## Post-merge Linear reconcile` exists to stop: a milestone naming H1 as a gate on two issues this stage had just moved to `Todo`. I made the third edit under step 2's own words, "wherever the merge made it false", and flagged it rather than assuming authorization. Revert is one scripted hunk if the FO disagrees.
- **A receipt comment creates relations after all.** Three unrequested `relatedTo` edges appeared from the comment alone. Yesterday's closeout recorded the opposite from a single observation and offered it as a narrowing; two observations now disagree, and the safer reading is the original rule — expect relations from any reference, in a comment as much as in a body.
- **One residual, deliberately unfixed here.** PR 2's `docs/promise-map.md` edit left line 73 at 133 columns against that file's ~97-column convention. A one-line rewrap for the next docs pass; this stage does not edit the repository.

### Summary

All six steps ran in order against the merged state, and this is the reconcile that released the board's structural item: five issues that had waited on H1 are now free, in `Todo`, with H1's `blocks` edges converted to `relatedTo` so the closed evidence stays reachable. Four writes landed — three milestones and the project's As-of block — each scripted from a fresh capture, asserted exactly-once, diffed to its authorized hunk count before sending, and compared to the intended body afterwards; zero emphasis damage on any of them. Two things are worth the FO's attention over the mechanics: the dispatch named two milestones where three were false, and the third was the one that would have contradicted this stage's own step 4; and the receipt comment added three `relatedTo` edges, which refutes the narrowing yesterday's closeout proposed.
