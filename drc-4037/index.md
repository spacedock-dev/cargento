...
## Stage Report: implementation (cycle 2, correction round 1)

- DONE: The authorized Linear writes performed FIRST and read back: the three drafted corrections to DRC-4037's body (A2.1-A2.3), the drafted milestone correction (B2), and one appended dated note on the closed DRC-4122 recording that its amendment section carries the wrong "six of the ten harnesses have no event adapter" predicate — appended only, its existing text unedited. Report every read-back discrepancy; they are the FO's to disposition.
  Discharged in the prior implementation round and reported there (see **Discrepancies on read-back** above, plus the Addendum's further set). The authorized correction package for this round routes R2-R7 only and authorizes no Linear write, so none was made and nothing was re-read.
- DONE: PR 1 built and opened, touching NO file under `cargento_runtime/web/`: the section promoted from `docs/plans/git-probe-security-scope.md` into `SECURITY.md` byte-identically, the two intro amendments the document names applied, the plan document DELETED in the same commit that promotes it, the probe wired to `session_ended`, `--no-git` added at all six `--no-spacedock` sites, `dirty` and `changed` published as nullable on every row, and the doc updates. Test-first: write the failing test, watch it fail for the right reason, and say what it asserted before it passed.
  PR **#239**, head **`eff93f6`**, still open, `mergeStateStatus: BLOCKED` on the review. Three commits added; nothing under `web/`. Every correction was watched to fail first: R2 produced `{'dirty': False, 'changed': 0}` where `{}` was required; R6's spy showed the stripped call site while the constant-only test stayed green; R4 reproduced the reviewer's exact `2 errors, 5 skips`.
- DONE: The two constraints the merged contract does not name are satisfied and proven, not assumed: the probe must NOT run synchronously on the event-ingress path ... and `session_ended` mints no overlay, so the result needs `finished_at`'s side-channel pattern restated in all three `patch.update` blocks of the reducer. Run the full canonical pre-PR suite from AGENTS.md ... measure the surface against the estimate before opening the PR, review the diff in the worktree first, and report the PR number and head SHA.
  AC3's two oracles are untouched and still green. The second constraint is the one that had shipped broken and is now fixed and pinned (below). Full suite green twice: 2005 tests, 1 skip; scripts 192 tests; ruff, `ruff format --check`, `mypy --strict`, `lint_embedded`, `validate_plugins`, `bump_version --current`, `claude plugin validate --strict`, `agy plugin validate` all clean; coverage 90.8% against `fail_under = 73`; zero version-field movement since the merge base. Diff reviewed in the worktree before the push, which produced one fix of its own (`eff93f6`).

### What the corrections were, and what now catches each

- **R2 (`78fe651`).** `_side_channel_patch`'s guard was `bool(finished_at) and …`, vacuously false exactly when `_mark_finished` has popped the mark, so `session_ended` then `turn_started` left the reading unguarded and a lapsed WORKING overlay republished a clean tree as `dirty: false` permanently. The reading now rides the mark's presence as well as its freshness. A second door was found while fixing it and is closed too: with the reading retained, the next `turn_stopped` re-pairs it with a stop it was not taken at, so `_mark_finished` now discards the reading alongside the mark. Caught by `test_a_reading_with_no_stop_behind_it_is_not_published` and `test_a_lapsed_working_overlay_leaves_the_reading_unpublished` (fail on reverting `fresh` to `not stale`), and by the three `..._discards_its_reading` coordinator tests (fail on removing the `_mark_finished` pop).
- **R3 (`78fe651`).** Both mutations the review measured green now fail. Deleting all six `dirty`/`changed` lines from the three `patch.update` blocks fails `test_a_live_{working,gate,idle}_overlay_nulls_the_reading` — each asserts `patch["state"]` first, so a branch that never ran fails loudly rather than passing on an absent key. Forcing `aggregate`'s `git=` to None fails `test_a_probed_row_reaches_the_published_bytes_carrying_no_pathname`. Verified by re-running both mutations against the full 265-test set for the three modules.
- **R4 (`78fe651`).** `GitProbeReadingTest` gained the `skipIf` its two siblings had. With `PATH` pointed at a shim: **0 errors, 7 skips** after, against the reviewer's 2 errors and 5 skips before. Removing the decorator restores 2 errors, so the guard is pinned by measurement rather than assumed.
- **R5 (`78fe651`, `eff93f6`).** AC5's named oracle now exists: a distinctive filename **and** a distinctive directory go through the real porcelain parse into a real collection, and the absence is asserted on `collect_json()`'s bytes. The diff review changed the fixture's directory from `another` to a distinctive literal, because a plausible English word in an absence assertion flakes rather than catches.
- **R6 (`78fe651`).** A spy pins what `probe()` hands `runner`: the argv, the cwd, the timeout, the closed stdin, and the absence of `shell=`. Stripping both flags at the call site fails it while `test_the_probe_is_exactly_the_one_bounded_command` stays green, which is the review's claim reproduced. Deliberately undecorated, so the pin holds where the behavioural tests skip.
- **R7 (`78fe651`).** Both hazard tests run a positive control first, in a repository of its own, and `skipTest` with a reason where the control cannot arm the mechanism. Controls are spelled as literals rather than filtered from `GIT_STATUS_ARGV`, so a change to that constant cannot turn a control into a second copy of the probe. The fsmonitor script is written with `newline=""`, which fixes the `#!/bin/sh\r` shebang the reviewer widened the disclosure to. Both controls fired on macOS today, so both mechanisms are confirmed armed here rather than assumed.
- **R1 untouched.** No change to `SECURITY.md`, to the invariant wording, or to the feature's bounds. Nothing above required one.

### One thing tried and dropped, recorded rather than shipped

Clearing the reading on **every** `session_ended` looked like the tighter rule. It is unpinnable: the resume already discards the reading before any second end can reach it, so no test distinguishes the two, and two ends with no turn between them (`/clear` then exit) would discard a reading that is still accurate. Removed rather than shipped unpinned, which is R3's own lesson applied to the fix for R2. The reasoning is in `docs/design-needs-input.md` N-9 (`1d42ff5`), the section that already owns the lifecycle of state the coordinator holds outside the overlay ledger.

### Surface versus the re-declared estimate — beyond tolerance on one axis, the FO's to disposition

Measured `git diff --numstat 701b7f0..eff93f6`: **25 files, +1315 / −118**, against the re-declared **24 / +918 / −114** at ±35%.

| Axis | Declared | Actual | Delta | Within ±35% |
| --- | --- | --- | --- | --- |
| Files | 24 | 25 | +4.2% | yes |
| Added | 918 | 1315 | **+43.2%** | **no** |
| Deleted | 114 | 118 | +3.5% | yes |

The correction round itself is **7 files, +414 / −31**, of which the runtime is +32 / −7 and roughly four lines are functional (`fresh`, and one `pop`); the rest is +363 of tests and +19 of design prose. Cumulatively the branch is runtime 8 files / +335, tests 9 files / +885, docs and scripts 8 files / +85 / −104. So the added-lines overrun is **the same axis and the same cause the captain accepted once**: test volume, and this time volume the review itself demanded, since R3's finding is precisely that the guards were unpinned and R5's is that AC5's named oracle did not exist. Trimming to fit would delete the oracles this round was routed here to add. Surfacing the figure rather than acting on it, per the stage definition: tolerance is captain-owned.

The one new file in the set is `docs/design-needs-input.md`, taking the count from 24 to 25.
