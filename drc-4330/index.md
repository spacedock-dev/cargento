...
- DONE: The six `## Post-merge Linear reconcile` steps run in order against the merged state (PR #258, merge commit `950822b`)
  Merge verified on `main` first, at the SHA rather than from `gh pr diff`: `git show --stat 950822b` is one file, `docs/plans/history-store-security-scope.md`, 139 insertions, 2026-09-03T04:41:18Z.
  Step 1 — verified, not set. Already `Done`, `completedAt` 2026-09-03T04:41:20.389Z, two seconds after the merge, so GitHub automation moved it and this stage made no state write.
  Step 2 — two edits to `Move up a level`, built by `/tmp/drc4330/ms.py` from a fresh `get_milestone` capture. Each target passage asserted exactly once (the script exits non-zero on any other count), diff printed exactly 2 hunks for 2 authorized edits, both deletions. The "What remains" line came off because the section lists what remains; the "Waits on" line came off because a shipped gate gates nothing. The near-miss sharing its prefix did not match the first assertion — anchoring on the full line including ": The history store's security contract" is what separated them.
  Step 3 — three figures changed, re-derived rather than adjusted. Open issues 44 to 43, defects or engineering work 20 to 19, by-move `none` 12 to 11; As-of heading to 2026-09-03. Derived from one state-filtered `list_issues` per status type and tallied by `/tmp/drc4330/asof.py`, which asserts the by-move tally covers every open issue. Everything else re-derived identical and was left untouched: 24 open board items against 24 closed, sixteen shipped and eight cancelled, per-release row r1 0, r2 2, r3 10, later 12. Board items were identified as the open issues carrying a `release:*` label, which reproduces the block's own 24 and its r3 set exactly (H2, B2, H1, C6, C1, D9, D10, E7, B5, F1).
  Step 4 — H1 (DRC-4044) confirmed free and already in `Todo`, so no state write. Its own five `blocks` edges on A6, D10, F2, F3 and G4 are H1's, not this issue's, and were left alone.
  Step 5 — the `blocks` edge to DRC-4044 removed and `relatedTo` added in its place, the blocked side being still open. Every other edge on both issues unchanged.
  Step 6 — labels are `move:none` and `journey:mid-flight`; a `none` merge changes no promise wording, so `docs/promise-map.md`, the README lede and the project's promise table all stand, and nothing is handed to the next `sync-docs` pass. Said explicitly in the receipt rather than left implied.
- DONE: A receipt comment posted on DRC-4330 naming all six edits, PR #258 and merge commit `950822b`, with the relation set read back after every body or comment write and every emphasis-boundary move or unrequested `relatedTo` edge reported rather than repaired
  Comment `929dd034-4699-455d-b1a2-c92876352b6c`, posted 2026-09-03T04:50:05Z; the returned body is byte-identical to what was sent, so the receipt took no emphasis damage.
  Relations read back twice — after step 5 and again after the comment. Both times: `blocks: []`, `blockedBy: []`, `relatedTo: [DRC-4044, DRC-4329, DRC-4327, DRC-4326, DRC-4234]`. No unrequested edge appeared, and no `blocks` or `blockedBy` edge anywhere, so nothing is Material.
  Zero emphasis-boundary moves to report, on any of the three writes. On the milestone the trigger was absent rather than avoided: that description contains no code span at all. The project description does contain them, and its two bold runs both survived a `patch` intact because neither ends immediately before one.
- DONE: `## Stage Report: review (post-merge closeout)` written on this entity — a new heading, never a `###` under the cycle-2 review report — with DONE/SKIPPED/FAILED per item above and every evidence line on its own line; frontmatter untouched (`reconciled` is the FO's stamp) and `merge guard` not run
  This section, appended at the end of the entity after line 1144. Frontmatter not opened; `merge guard` not invoked; no worktree or branch recreated.

### Two things for the FO

- **Step 5's call returned an error while fully succeeding.** `save_issue` with `removeBlocks` plus `relatedTo` returned `Error: Failed to remove 1 relation(s)`. Read-back showed the whole write had landed — `blocks` empty on this issue, `blockedBy` empty on H1, the `relatedTo` edge present both ways. No retry was sent. This is the fourth observation of the behaviour the workflow rules already record, now on a relations-only write with no body patch in the call, which narrows it further: the error does not require a body change to appear.
- **The milestone's shipped-item convention is not uniform, and I picked the narrower reading.** `Don't burn capacity` carries a `## What shipped, and what it changed here` section; `Nothing dies quietly` simply dropped E4 from "What remains" on merge and added nothing. I followed the second, because the dispatch authorized two edits and a new section would have been a third hunk the rule says to stop on. If the captain wants the shipped line preserved as a dated record instead, that is one further authorized edit rather than a correction of this one.

### Summary

All six reconcile steps ran against the merged state, and this is the step that actually released H1: its `blocks` edge became `relatedTo`, its blocked-by set is now empty, and it stands free in `Todo` with only its own five dependents still gated on it. Three of the six needed no write — the issue was already `Done` by GitHub automation, H1 was already in `Todo`, and a `move:none` merge owes no promise wording — and each is recorded with why it is a no-op rather than left to look like an oversight. The two writes that did happen were both scripted from fresh captures with exactly-once assertions and diffed before sending, the milestone at exactly two hunks and the project at two anchored replacements, and all three writes including the receipt came back with no emphasis damage. The one thing worth the FO's attention is that step 5's call reported a failure it had not suffered, on a relations-only write, which sharpens the existing record of that error.
