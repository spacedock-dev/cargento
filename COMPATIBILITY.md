...
- Needs-input detection reaches two harnesses of the ten. Claude Code has three sources; Codex has one, its bundled `PermissionRequest` hook, which was measured firing in an interactive session with the approval prompt still on screen. The other eight expose no signal a passive reader can use, so the payload declares the capability per harness and the page marks those rows rather than letting their silence read as an all-clear. One per-harness limit rides with that: a Codex row can say a gate is open without saying which, because the event envelope drops the tool name at the hook. Delivery is no longer a second one: a Codex gate raises a popup on every platform Claude's does, on the same terms. The ask lane is a different mechanism and is not detection at all, so it does not follow that line: an `ask_operator` question is raised by the session itself over MCP, which means any harness that can register the stdio server can put a question on the board no matter what its store records. Which harnesses those are is the MCP row above.
