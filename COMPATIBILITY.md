...
| MCP | Not registered by the plugin; the user adds `mcp_server.py` to their own `mcp_servers` table. Gate and hold measured attended on codex-cli 0.146.1 | Registered: `mcpServers` in `.claude-plugin/plugin.json`. Gate and hold measured attended on 2.1.239, and the plugin-declared path is measured too: loaded with `--plugin-dir`, it produces the tool `mcp__plugin_cargento_cargento__ask_operator`, and one round trip ran end to end through it | Not registered. Antigravity's MCP path takes a `serverUrl` only and cannot express a stdio server without a validator change | Not registered. The Gemini extension root carries hooks only, so there is no manifest surface for it | One stdio server, `cargento/skills/cargento/mcp_server.py`, serving one tool, `ask_operator`. Session data still comes from reading local stores; the server exists so a session can ask the reader a question and wait. Captures in `docs/captures/{claude,codex}/mcp-ask-gate-*.jsonl`. [`SECURITY.md`](SECURITY.md) owns the contract and the standing permission it needs, [`docs/design-ask-lane.md`](docs/design-ask-lane.md) the rejected alternatives |
...
- Needs-input detection exists only for Claude Code sessions. Other harnesses expose no equivalent signal in their local stores. The ask lane is a different mechanism and is not detection at all, so it does not follow that line: an `ask_operator` question is raised by the session itself over MCP, which means any harness that can register the stdio server can put a question on the board no matter what its store records. Which harnesses those are is the MCP row above.
...
<!-- docs-synced-through: ffd2fe4 (2026-08-21) -->
