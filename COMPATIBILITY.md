...
|---|---|---|---|---|---|
| Package marker | `.codex-plugin/plugin.json` | `.claude-plugin/plugin.json` | `plugin.json` | `../cargento-gemini/gemini-extension.json` | Each runtime receives its native root/manifest shape. Gemini's and Droid's (`../cargento-droid/.factory-plugin/plugin.json`) are separate roots: see Hooks below |
...
The floor is restated in eleven other places across eight files, which must all move together:
`README.md`, `CONTRIBUTING.md` (three times, in the prerequisites, in the `runtime-floor` bullet of
the quality-gate list, and in the dashboard implementation constraints),
`cargento/skills/cargento/SKILL.md`, `docs/promise-map.md`, `.github/PULL_REQUEST_TEMPLATE.md`,
`AGENTS.md`'s quality-gate paragraph, and `pyproject.toml` (`[tool.ruff] target-version` and
`[tool.mypy] python_version`), plus the Python 3.11 direct-launch smoke in
`.github/workflows/quality-gate.yml`. The documentation-matches-code test guards three of them, the
`SKILL.md` copy and both `pyproject.toml` fields, and the runtime-floor job exercises the shipped
entry point on Python 3.11. The rest are on you.
...
| Gone-quiet popup, browser (tab open) | no (the server delivers it) | yes | yes | yes (host browser) |
| Gone-quiet popup, native (no tab) | yes (`osascript`, Claude only) | not yet | not yet | not yet |
| Terminal raise from the gate queue | yes (tmux, one attached client) | not yet | not yet | not yet |
...
Exactly one layer delivers a given popup: the server where it has a native backend, the page otherwise. `/api/data` reports which as `native_notify`. The split covers an arriving `ask_operator` question as well as a needs-input transition, which is why the ask rows above read the same way. It covers a session going quiet too, with one asymmetry the rows record and the sentence cannot: the native layer fires off Claude's own idle hook and so reaches only Claude, while the page has no hook and fires on any harness's row when it stops looking busy. Those are near the same moment and not the same event, so the wording differs by layer. Both layers are harness-independent: the server decides once per collected row, whatever harness produced it, so a Codex gate is delivered on macOS exactly as a Claude one is. That is new. The server used to fire only from Claude's collector, which left a gate on any other harness with no popup on macOS at all while the page stood down for it.

The terminal raise is recorded only on the platform the case was measured on, so a target is never stored off macOS and every row elsewhere reports no reachable terminal. Linux needs more than a measurement: the device grammar the contract fixes admits `/dev/ttys004` and refuses `/dev/pts/3`, so widening it is a contract change and not a platform port. tmux is the only arrangement named, one attached client is required (`switch-client` moves the window every client of that session displays, so two clients means somebody else's view moves too), and a session that predates the running dashboard has no recorded target. `--no-focus` turns the whole thing off, and so does `--no-events`.
...
The unasked reading lane raises through the SERVER side of this same lane and records the same
outcome, so everything above about a server-side raise applies to a departure. The browser half does
not: the page raises on edges it computes from the payload, and a departure is not one of them, so on
Linux and Windows a departure reaches the delivery record and no banner. It also needs a working
`codex` CLI on the machine, which is the dependency a reader-requested reading already has, and it is
off by default on every platform.
...
restriction. Shutdown can still remove its own registration file on every platform: a separate
bounded regular-file read compares the server generation without trusting any capabilities.
...
- Needs-input detection reaches six harnesses of the ten. Claude Code has four sources of its own, and the fourth is about a subagent rather than the session: a member listed in the teams registry that has written no transcript was dispatched and never started, which is the shape an agent parked on a startup permission prompt takes on disk. That one is bounded at both ends, since the registry stamps a join and never ages it: a member is read this way only once it has been registered longer than a healthy agent takes to start, and only while its session is still inside the activity window. Nothing detects a gate that opens after a member has started. The registry does now carry one per-member flag, `isActive`, which Claude Code 2.1.259 sets false on a member it has finished with rather than pruning it, and that separates finished from live; it says nothing about whether a live member is blocked or busy, and an inbox never carries a reply. The flag is read only to demote a member transcript freshness already reads as running, never to promote one, because whether a hard-killed pane clears it is unmeasured. Codex has one, its bundled `PermissionRequest` hook, which was measured firing in an interactive session with the approval prompt still on screen. That hook is the whole of its gate reporting, so the capability reaches only an operator who runs Codex with approvals enabled: with approvals off no prompt opens, the hook never fires, and a quiet Codex row says only that nothing asked. The payload declares that condition beside the capability and the coverage line prints it, because nothing can read it off a config file: `approval_policy` is overridable per invocation and per project, so a global reading cannot say whether a given session will ask. Copilot needs no hook at all: its CLI writes `permission.requested` to `events.jsonl` when the dialog opens and `permission.completed` only when the human answers, joined on `data.requestId`, so the collector reads an unanswered request as a standing gate. That was measured on 1.0.78 across six interactive sessions, with the request on disk in the first frame the dialog was visible and gates standing 23 to 48 seconds. Cursor takes Copilot's route on a different store, and also needs no hook: a standing tool-call gate leaves `pendingToolExecutionContracts` non-empty on the newest blob of the chat's SQLite store, and answering it appends a blob with the map emptied. That was measured on 2026.08.11-e8db854 across four interactive `allowlist`-mode sessions, with gates standing 15 to 244 seconds and the record readable through the collector's existing read-only connection. OpenCode adds a per-project passive permission plugin; Pi adds extension UI prompt callbacks on 0.85.1 for persisted sessions, excluding startup trust. Both require events enabled, disclose their installation condition and lose standing observations on restart. The other four have no shipped gate path, so the payload declares the capability per harness and the page marks those rows rather than letting their silence read as an all-clear. Three per-harness limits ride with that: a Codex row can say a gate is open without saying which, because the event envelope drops the tool name at the hook; a Copilot row reports only whether a command or a URL is being asked about, because the command text and the model's stated reason are the class of value Cargento does not read; and a Cursor row says only that a permission request is open, since the three fields that would name the tool sit in the same contract entry the reader deliberately stops short of. A Copilot session directory that holds no `events.jsonl` is not a row in any state, so it cannot report a gate either; 3 of 7 directories on the probe machine were in that position. And a Copilot gate has no liveness signal behind it: the store records the question and never records the process going away, so a session killed at its dialog reads the same as a person who walked away from one. A prompt typed into the session retires the request, which covers a session that was resumed and is working again; a session that never comes back keeps its red row until it ages out of the activity window. Cursor has the same hole and a sharper version of it: its store is append-only, so a session abandoned at a gate stays pending in the file forever, and 2 of 10 untouched stores on the probe machine still read as waiting 29.4 hours after their process exited. Its wait is therefore published only while the row is inside the activity window, which is the same freshness its title is already gated on. A Cursor subagent's gate is not read at all: a child folds onto its parent's card as a pill and its own store is never published. Delivery is not a limit: a Codex, Copilot or Cursor gate raises a popup on every platform Claude's does, on the same terms. The ask lane is a different mechanism and is not detection at all, so it does not follow that line: an `ask_operator` question is raised by the session itself over MCP, which means any harness that can register the stdio server can put a question on the board no matter what its store records. Which harnesses those are is the MCP row above.
- Re-entry commands reach two harnesses of the ten, and only where the verb was read off the installed CLI's own help rather than off its documentation: `claude --resume <session-id>` on Claude Code 2.1.261, and `codex resume <SESSION_ID>` on Codex 0.153.4. A gate-queue row for either one carries a control that copies that command; the other eight publish no session id their CLI would accept, so their rows carry no control, because a guessed verb costs the reader a failed command on top of the hunt it was meant to replace. The id is not always the one the row displays. Claude's is the whole transcript stem, since the CLI refuses the eight-character prefix a Claude row shows, and Codex's is the rollout id it already publishes. Neither CLI lets a second process onto a live conversation, and both refuse in a way that names what to do next, so the control ships with no warning attached; [`docs/design-next-ui.md`](docs/design-next-ui.md) records what each refusal looks like and [`SECURITY.md`](SECURITY.md) owns the grammar the copied token has to satisfy.
- Turn-end events reach only the four harnesses with turn-end hooks, Claude Code, Codex, Antigravity and Gemini CLI, each through the hooks bundled for it, so only their rows can tell a session that finished and went unread from one still waiting on a reply that never came. The other six (Pi, Copilot, OpenCode, Cursor, Goose and Droid) are read by scanning their stores, and their rows say the answer cannot be known there rather than guessing at it. No collector may infer a completion.
- A reader-requested reading has two reaches and neither is the whole board. A final reading rests on an observed session end, and `ended_at` is patched only by the event overlay reducer, so only the four turn-end-hook harnesses above can ever carry one. A scan-only row still reads mid-flight while it is running and claims no finality; idle, it says no end can be observed there rather than that nothing finished. The expected-output constraint is narrower again: only Pi publishes a demonstrated work result, so on the other nine harnesses that constraint is never put to the model at all rather than answered weakly. The lane is the second caller of the one bounded `codex exec` subprocess the observer model uses, so on every platform it needs an absolute `codex` on the server's PATH and reports the model unavailable without one, having spent nothing. `--observer-model` remains off by default. The accepted case review enables the control when that model option is on. Every model call runs in a process group of its own, and a timeout, a shutdown or a reader's Cancel kills that group: a POSIX session on macOS and Linux, where a helper that leaves the group (by `setsid` or `setpgid`) is not reached, and on Windows a kill-on-close Job Object the child is placed in while suspended, which only the native Windows `platform-tests` job exercises. The exit is watched without reaping, by `waitid` on Linux and a kqueue exit filter on macOS, so the group can be swept while its leader still holds the id; the reap and the end of signalling are one step under the group's lock, and an exit neither can watch is polled rather than guessed. [SECURITY.md](SECURITY.md) owns what the call may send, and [docs/design-reading-a-session.md](docs/design-reading-a-session.md) the shape contract the answer has to satisfy.
- Spacedock workflow detection covers Claude Code, Pi and Codex. Claude rests on an `agentSetting` in the first transcript records plus the first officer's boot output. Pi writes no `agentSetting`, so a session is taken to be a first officer when its transcript carries the boot envelope itself; that costs a transcript scan per Pi session, where Claude pays one cached lookup and skips the scan entirely. Pi gets no ensign role and no bolded entity, because it reports no workers. Codex also attaches a workflow from a supported command-output boot result, including its captured fulfilled wrapper with integer exit code zero and one complete boot object. Its badge follows that evidence; no Codex ensign role is inferred. Pi forks and clones remain independent rows, not subagents.
...
- Loop detection, the failed tool calls behind a long turn's explanation, is also Claude Code only. That covers all three of its readings: the consecutive run, the request total, and a request in which nothing succeeded. Claude is the only harness that records whether a tool call failed, so every other row reads as not measured rather than as nothing failed, and none of them ever carries the signal. `cargento/skills/cargento/SKILL.md` owns what the signal means and which record each of the other harnesses is missing.
