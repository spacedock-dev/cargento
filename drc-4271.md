...
4. The probe scripts behind the tables above re-run and land within the stated ranges. *Verified by:*
   `/tmp/drc4271_probe.py`, `/tmp/drc4271_cov.py`, `/tmp/drc4271_sim.py`. **Declared machine
   dependency:** these read this machine's live `~/.claude/projects` store, so the *numbers* are not
   portable; the *method* is, and any machine's own store re-derives them. They are deliberately not
   committed as tests — a test pinned to one machine's session history would be a false oracle.
