...
This is the contribution we most want. Each harness is one registry entry: a key, a display label, a cheap discovery predicate, and a collector function. The runtime contract is `HarnessSpec` in `cargento_runtime/aggregate.py`, whose discovery takes `(config, state)` and whose collector takes `(config, state, now, window_hours, show_all)`. While the staged split is in progress the rows live in `_HARNESS_ROWS` in `server.py`; a row names either a collector module under `cargento_runtime/collectors/` that already speaks that contract, or a `_Legacy` pair that still reads module globals and is wrapped to it. New harnesses should be a collector module, so follow `collectors/codex.py`. Study `collectors/droid.py` (JSONL-based) or `collectors/goose.py` (SQLite-based) as templates. Requirements:
