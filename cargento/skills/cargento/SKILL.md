...
|---|---|---|
| **Needs input** (red, popup on Claude) | An agent is blocked on the human | Best effort, never guaranteed, and only two harnesses can report it at all. **Claude** has three sources that cover different things rather than ranking cleanly: the bundled `PermissionRequest` hook is the main one for tool gates, seen directly for `ExitPlanMode` and in the wild for `AskUserQuestion`; an actionable Notification-hook POST is the *only* source for an MCP elicitation or a worker's permission or network request; and a pending input tool seen in the transcript is an opportunistic extra, because Claude Code flushes that record on its own schedule and sometimes not until the gate has been answered. **Codex** has one, its own bundled `PermissionRequest` hook, measured firing interactively with the prompt still on screen. A Codex row says a gate is open and cannot say which, because the event envelope drops the tool name at the hook, and it raises no popup — see Notifications. The other eight harnesses have no gate detection, and the harness strip marks each of them `no gate detection` so a quiet row there cannot be read as an all-clear |
...
**Known gap:** a Codex gate raises no popup on macOS. The two layers divide by platform, and the division assumes the server raised the alert natively wherever it can — which is true of Claude and of nothing else, because only Claude's collector calls the native notifier. So on macOS the server does not fire for a Codex row and the page stands down expecting it to have. On Linux and Windows the page owns the popup and a Codex gate does raise one. Either way the board shows the row, the band lists it and the tile counts it.
...
Eight events are registered, and each one was measured firing before it was mapped: `SessionStart`, `UserPromptSubmit`, `PermissionRequest`, `PostToolUse`, `Stop`, `PostCompact`, `SubagentStart` and `SubagentStop`. `PreToolUse` fires too and is deliberately not used, because `PostToolUse` reports the same turn once the store has actually changed.

`PermissionRequest` is the one that makes a Codex row go red, and it is worth knowing what it does and does not do. Codex blocks the tool call on the hook and applies whatever the hook returns, so this is the one event where a reporting adapter could gate the user's own work. It cannot here: the adapter writes nothing to stdout on any path, and returning nothing is how Codex is told the hook has no opinion, so the approval prompt reaches the user exactly as it would with no hook installed. Cargento never answers a Codex gate. What it costs is one short-lived process on the approval path, which is a path where the agent has already stopped.
...
Two honest limits. The conversation id is blank until a conversation exists, which is measured: the return to Idle often cannot be reported, so the Working state carries a deadline and the collector decides again after it lapses. The other is weaker than it used to read here. No status-line payload Cargento has recorded carried a confirmation-pending field, so an Antigravity row does not report Needs input from this path — but every recorded session was a non-interactive one, where no tool confirmation can be pending, so that is an absence of evidence rather than a measured no. Treat it as unsettled.
...
- **A quiet row is not proof nothing is waiting, and on eight harnesses it is not even weak evidence.** Only Claude and Codex can report a gate; the other eight have no signal to send, so their rows are silent whether or not a person is being asked. The strip marks those `no gate detection`, and a `Needs you` tile reading zero names them rather than saying nothing is waiting, because "nothing is waiting" and "nothing here could tell you" are different facts that used to render identically. Where detection does exist it is still best effort: a session sitting at a gate can read Working for a while before the red band appears, so absence of the band is not evidence that nothing is blocked. The band is not proof either — an unrecognised notification type is deliberately treated as actionable so a new kind of prompt cannot vanish, and that same rule can raise the band for something that turns out not to need you. A question raised through the ask lane is the exception to all of it: any harness can register one, so those appear on any row.
