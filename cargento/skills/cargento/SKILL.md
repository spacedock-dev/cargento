...
- `~/.claude/projects/*/<session-uuid>/subagents/agent-*.jsonl` + `.meta.json` — running Claude subagents (fresh mtime = running; named by meta `name`/`description`/`agentType`)
...
Stdlib-only, Python 3.8+, no dependencies. Run it in the background (`run_in_background: true`), confirm it responds (`curl -s http://localhost:4553/api/data | head -c 200`), then open the UI:
