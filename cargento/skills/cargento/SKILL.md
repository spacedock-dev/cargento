...
- Spacedock workflows: a Claude session launched by Spacedock carries an `agentSetting` of `spacedock:first-officer` or `spacedock:ensign` in its first transcript records, which is how the role badge appears. A first officer also records its `spacedock status --boot` output, naming each workflow directory absolutely and listing each entity's current stage; the ordered stage list is then read from that workflow's `README.md` frontmatter. This is the only file Cargento reads inside a project — see the repository's SECURITY.md for the contract, and `--no-spacedock` to disable it
...
- **Spacedock stage strip** = one line per in-flight entity, showing that workflow's stages in declaration order with the entity's current stage highlighted. A bold entity name means a worker for it is running now; a plain one comes from the boot snapshot and may have moved on. A long workflow is windowed around the current stage, with `…` standing in for the stages elided. No strip appears when the boot output is outside the scanned head of a long transcript, when the workflow README cannot be read, or when its frontmatter uses a construct the reader does not model — it renders nothing rather than a guess.
