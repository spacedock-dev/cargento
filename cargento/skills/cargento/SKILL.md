...
Both layers notify on the *transition* into needs-input, not on every refresh a session spends blocked. A question is different: it has no prior state, so both layers notify on its arrival, once per question, keyed on the question's own id. Any harness that can register the stdio MCP server can raise one — the ask lane is not needs-input detection and is not Claude-only.
