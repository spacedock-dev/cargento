...
Three paths manage needs-input state, and a fourth alert — a question registered through the ask lane — is not needs-input state at all. Two layers can deliver the popup, and **exactly one of them fires for any one alert**, so nobody is notified twice:
