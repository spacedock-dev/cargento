...
- **Session id** = enough leading characters of the session's id to be unique among that harness's rows, at least 8. Codex hands out time-ordered ids, so agents launched together share a long prefix and a fixed 8 characters showed several distinct sessions as one repeated row; the width grows only for the harness that needs it.
