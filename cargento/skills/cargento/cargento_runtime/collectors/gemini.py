"""Gemini CLI and Antigravity CLI collection.

One registry key covers both source families: legacy Gemini CLI JSONL
transcripts and current Antigravity CLI per-conversation SQLite stores. They
share a discovery result, an error boundary and a display label, so they are
one collector rather than two.
"""
...
def _collect_antigravity(
    ...
    for db in dbs:
        ...
        session = sessions.base_session("gemini", sid, project)
        session.update(
            {
                "title": prompt.split("\n")[0][:80] or None,
...
def collect(
    ...
    for sid, (mtime, fp) in found.items():
        ...
        s.update(
            {
                ...
                "turn": turns.turn_progress(
                    turns.scan_turns(config, state, fp, "gemini") if info else None,
                    session_state,
                    now,
                    config,
                ),
        ...
        out.append(s)
    out.extend(_collect_antigravity(config, state, now, window_hours, show_all))
    return out


def discover(config: RuntimeConfig, _state: RuntimeState) -> bool:
    """Matches both supported Gemini stores: legacy Gemini CLI JSONL and
    current Antigravity CLI per-conversation SQLite databases."""
    return bool(
        runtime_io.glob_stores(config, "gemini.tmp", "*", "chats", "session-*.jsonl")
    ) or bool(runtime_io.glob_under(_conversations_dir(config), "*.db"))
