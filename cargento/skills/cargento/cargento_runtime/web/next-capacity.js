...
/* Slots that can carry a pace, in the order the strip considers them. `month` is
   absent for the reason `quota.PACE_SLOTS` omits it: Cursor meters money over a
...
function nextUsageConsent(){
  ...
  try{
    const raw = localStorage.getItem(NEXT_USAGE_CONSENT_KEY);
    return raw === "granted" || raw === "declined" ? raw : null;
  }catch(_error){
    /* A private window, or storage the browser refuses. Unanswered is the safe
       reading: it withholds the fetch rather than performing one nobody agreed
       to, which is the direction this whole surface has to fail in. */
    return null;
  }
}
...
function nextSetUsageConsent(answer){
  try{
    localStorage.setItem(NEXT_USAGE_CONSENT_KEY, answer === "granted" ? "granted" : "declined");
  }catch(_error){ /* no storage; the answer lasts this page only */ }
}
...
function nextCapacityWindow(entry, slot, generated){
  ...
  let elapsed = null;
  if(windowSec != null && windowSec > 0 && remainingSec != null){
    /* Clamped, because a reset stamp can sit outside its own window: a stale
       disk snapshot can put the reset in the past, and a vendor clock can run
       ahead of ours. Clamping keeps the tick on the bar; it never invents one,
       since a missing length or reset leaves `elapsed` null instead. */
  ...
  const windowPacePerMin = elapsed != null && elapsed > 0 && windowSec != null && windowSec > 0
    ...
    : null;
  const recentPacePerMin = recent && nextNumber(recent.pctPerMin) != null
    ? Math.max(0, nextNumber(recent.pctPerMin))
    : null;
  ...
  return {
    ...
    windowMinutesLeft: minutesAt(windowPacePerMin),
...
function nextCapacityClock(seconds){
  /* Wall-clock words for an instant this many seconds ahead, matching the
     countdown column's own formatting elsewhere on the board. */
...
function nextCapacityEnds(row){
  ...
  if(row.remainingSec != null && row.windowMinutesLeft * 60 >= row.remainingSec){
    ...
    return `<span class="next-capacity-slack">lasts, ~${Math.max(0, spare)}% spare</span>`;
  }
  const clock = nextCapacityClock(row.windowMinutesLeft * 60);
  const basis = row.thinBasis ? ` <em>on ${nextCapacityDuration(row.elapsed * row.windowSec)}</em>` : "";
  return `${esc(clock)}${basis}`;
}

function nextCapacityRow(row){
  ...
  return '<div class="next-capacity-row" data-next-capacity-row=' +
    ...
    `<div class="next-capacity-pct">${row.pct}%</div>` +
    nextCapacityBar(row) +
    `<div class="next-capacity-pace${row.paceRatio != null && row.paceRatio > 1 ? " hot" : ""}">` +
    `${pace}</div>` +
    `<div class="next-capacity-ends">${nextCapacityEnds(row)}</div>` +
    `<div class="next-capacity-resets">${resets}</div>` +
    "</div>";
}
...
function nextCapacityProspect(row, workingCount, projectSpread){
  ...
  const stale = row.recentMinutesLeft == null && row.asOf != null
...
function nextCapacityView(payload){
  ...
  const untimed = rows.filter(row => row.windowMinutesLeft == null).length;
  const more = rest > 0
...
function nextCapacityProjectSpread(payload){
  /* How long sessions in the busiest project have actually run, from the state
     transitions the history store already keeps. This is A6's shape-match, and
     it is keyed on project and duration rather than on a prompt: sizing an
     unstarted task from its text was the issue's original mechanism and it is
     the part the board itself called the part nobody does well. A project's own
  ...
  for(const record of history){
    ...
    if(at == null || !sid || !project) continue;
    const key = `${project} ${sid}`;
    const seen = spans.get(key);
    if(seen == null){
      spans.set(key, {first: at, last: at, project});
    }else{
      seen.first = Math.min(seen.first, at);
      seen.last = Math.max(seen.last, at);
    }
  }
  for(const span of spans.values()){
    const seconds = span.last - span.first;
    /* A session with one recorded transition has no measurable span. Counting it
       as zero would drag the median toward nothing, so it is left out and the
       count says how many sessions the figure rests on. */
    if(seconds <= 0) continue;
    if(!byProject.has(span.project)) byProject.set(span.project, []);
    byProject.get(span.project).push(seconds);
  }
  let best = null;
  ...
  return `Sessions in <b>${esc(best.project)}</b> have run ` +
    `${esc(nextCapacityDuration(sorted[0]))} to ${esc(nextCapacityDuration(sorted[sorted.length - 1]))}` +
    `, median <b>${esc(nextCapacityDuration(median))}</b>, from ${sorted.length} observed.`;
}
