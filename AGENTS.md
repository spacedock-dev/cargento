...
| `docs/captures/` | Recorded hook payload shapes from real harness sessions: the evidence behind any adapter gate marked measured. Field names and timings, plus two closed-vocabulary enums (`notification_type`, `reply`); never a value a person or a model wrote. |
