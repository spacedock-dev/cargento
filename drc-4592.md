...
--ac-scan` returns all seven `AC-1`..`AC-7` with their offline marks. Un-hyphenating any id, or
wrapping any bold label before its closing marker, drops that criterion from the result and the scan
still exits 0 — which is the silent partial-scan failure the stage definition warns about.
