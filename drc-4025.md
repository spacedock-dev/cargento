...
The six numbered criteria in the proposed issue body are the reviewable acceptance contract: real payload admission; fixed-label privacy and negative controls; source-honest session/Attention rendering; retention without coalescing loss; off-state and respawn behavior; and measured process-exit containment. Each declares offline/interactive proof and a concrete falsifier. Their implementation proofs remain owed. The measured 250 ms planned containment assertion must be evaluated under supported CI loads; it is not a universal scheduling guarantee.
