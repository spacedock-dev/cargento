...
| `antigravity/hooks-fire-1.1.18-macos.jsonl` | Whether Antigravity's plugin hooks fire at all. agy 1.1.18, macOS, a scratch plugin carrying only a root `hooks.json` and a recorder, installed with `agy plugin install` and uninstalled afterwards. A **negative** capture, and like `codex/permission-hook` what makes it worth keeping is the controls, because a hook that never runs and a probe that is broken look identical from the outside: the plugin loads (`hooks: 5 processed`), the recorder writes a record when invoked exactly as a hook command would invoke it, an absolute command path behaves no differently from a relative one, and an explicit `plugin enable` changes nothing. Three interactive sessions completing thirteen tool calls produced zero invocations, so the earlier `agy --print` result was not a print-mode artefact. Settles the firing half of DRC-4175, makes its decision half unreachable, and means Cargento's own shipped Antigravity hooks have never been live. |
