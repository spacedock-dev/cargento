...
| Project plan, raw status, earlier Course entries, other directions, and the tier-2 caveat bodies in the drift block | Restored independently by project and selected scope | `nextCockpitDisclosureAttr` in `next-cockpit.js`, which `nextCockpitWhy` reuses rather than registering a lane of its own |
