...
| An unsaved goal or expected output in `Held to` | Kept in memory across every redraw, and never written to browser storage. Bounded to the server's own cap, published as `annotate_cap` and 240 characters today. Escape drops the draft, which puts the saved revision back. The draft is kept whenever the words did not reach the store, which is two cases and not one: a save the server refuses, and a save it accepts while answering `persisted:false` | `nextCockpitHeldDrafts` in `next-cockpit.js`, keyed by harness, session id and field |
