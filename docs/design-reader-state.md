...
| Project plan, raw status, earlier Course entries, other directions, and the tier-2 caveat bodies on Held to | Restored independently by project and selected scope | `nextCockpitDisclosureAttr` in `next-cockpit.js`, which `nextCockpitWhy` reuses rather than registering a lane of its own |
