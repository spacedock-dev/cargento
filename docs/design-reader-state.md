...
by a test; the five that are not are named there as well, because a lane whose name carries neither
...
| The `Held to` save cue | Restored for 30 seconds and then dropped on the next redraw; a keystroke in the field clears it, and only the last 16 fields keep one | `nextCockpitHeldStates` in `next-cockpit.js`, stamped by `nextCockpitHeldMark`, read by `nextCockpitHeldCue`, and expiring at the same `NEXT_CONTROL_STATE_TTL_MS` the row controls use |
