...
| `reading_policy.py` | The remembered reader-requested permission and twelve-attempt rolling budget. SQLite transactions admit concurrent tabs and processes before the model seam; off and forget revoke permission without refunding unexpired attempts. A leaf over config and optional SQLite loading in `io`, independent of the goal-summary browser consent. |
