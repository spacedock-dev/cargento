...
| Module | Owns |
|---|---|
| `project_context.py` | Prototype project evidence, dispatch joins, and semantic projections |
...
| `annotations.py` | The goal and expected output the reader typed against one session, as immutable numbered revisions. Shaped on `dismissals.py` and a leaf on the same four for the same reason, and it departs from it in exactly one place: there is no watermark. A dismissal lapses when the session moves again, because "I have handled this" is answered by later activity; an annotation does not, because later activity is the thing it exists to be read against. Bounded by a session count and a revision count rather than by age, so a session still on the board cannot lose what was asked of it because time passed. `aggregate` puts the published shape on every row, and `http_api` mutates it. |
