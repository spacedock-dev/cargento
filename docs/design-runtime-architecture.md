...
| `state.py` | Mutable caches, locks, bounded-cache helpers, the server start stamp, and the runtime's published snapshot. Every cache keys on a file's `(mtime_ns, size)` and is bounded at `max_cache_entries`. The two instruction caches are honest about what they do not buy: the key moves on every event, so they never hit for a live session, and they exist for the idle and `?all=1` rows re-read on each refresh. |
