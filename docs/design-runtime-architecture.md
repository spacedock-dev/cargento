...
| `supervise.py` | The runner every model call goes through: `subprocess.run`'s keywords plus `on_spawn`, with the child in a process group of its own (a kill-on-close Job Object on Windows), the whole tree killed on a timeout, a refusal to signal the daemon's own group held at the one call to `killpg`, and `kill_all` for shutdown. A leaf that imports nothing from the runtime, so the kill guard depends on nothing else. `observer.codex_exec` and `observer.claude_exec` default to it, which is how the goal lane, the unasked lane and a pressed reading all get it. |
...
| `reading_policy.py` | The remembered reader-requested permission, one answer per provider, the tool-output grants keyed by provider and destination in a table of their own, and the twelve-attempt rolling budget they share. SQLite transactions admit concurrent tabs and processes before the model seam; off and forget revoke every provider without refunding unexpired attempts. The Codex answer keeps the original single-row table, so an answer saved before there were two providers reads as Codex's. A leaf over config and optional SQLite loading in `io`, independent of the goal-summary browser consent. |
