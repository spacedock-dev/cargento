...
| `web/next-cockpit.js` | Scope tree and switcher, the tab strip with its per-tab lede and its derived per-tab cue, recovery briefing, Now / Course / Decisions / Console panels, and browser-local context. Also the `Held to` tab, which appears only at session scope, in the order it renders: a lede saying what the tab is for, the two typed fields and their store round trip, the re-entry limits, the baseline question a later direction raises, the reading block and its shape contract, the departures it raised, the two landing axes, and last the observed record. |
