...
| `history.py` | The local history of what the server observed: the store's path, its bounded read, the append, the age-first then size-cap eviction, and the discard-on-unreadable read. A leaf over `config` alone, which is narrower than `dismissals.py` beside it (that one reaches `records` and `io`), because the collection lane writes this store continuously and must not be able to reach a module that could grow an edge back toward it. So `io.diag` is inlined and the diagnostic sink is a parameter. It also owns the recording lane's `Lane`, injected at assembly rather than reached through the overlay source, which is `None` forever under `--no-events`. Its bounds are a security contract, not a preference: see "Local history (the session history store)" in [SECURITY.md](../SECURITY.md). |
