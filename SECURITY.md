...
made the page ask, in that order, and `test_next_capacity.py` now binds every clause above to the
builder that emits the parameter, so neither half can go missing again without a red test.
